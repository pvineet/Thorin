...
def minibatch_parse(sentences, model, batch_size):
    ...
    for st in range(0, len(partial_parses), batch_size):
        end = st+batch_size if st+batch_size < len(partial_parses) else len(partial_parses)
        unfinished_parses = partial_parses[st : end]
        while unfinished_parses:
            i = 0
            transitions = model.predict(unfinished_parses)
            for transition, parse_obj in zip(transitions, unfinished_parses):
                parse_obj.parse_step(transition)
                if not parse_obj.buffer and len(parse_obj.stack) == 1:
                    unfinished_parses.remove(parse_obj)
...
def test_minibatch_parse():
    ...
    sentences = [["right", "arcs", "only", "again"],
                 ["right", "arcs", "only"],
    ...
    test_dependencies("minibatch_parse", deps[4],
